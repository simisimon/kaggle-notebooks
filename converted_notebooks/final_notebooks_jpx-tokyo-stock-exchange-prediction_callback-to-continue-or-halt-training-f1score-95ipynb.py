...
# ## [3. Trim the  dataframes to balance datasets](#trim) ##
...
train_dir=r'../input/traffic-signs-gtsrb-plus-162-custom-classes/Data_images/Train'
test_dir=r'../input/traffic-signs-gtsrb-plus-162-custom-classes/Data_images/'    
filepaths=[]
labels=[]
...
# read in Test_data.csv file and create test_df
test_path=r'../input/traffic-signs-gtsrb-plus-162-custom-classes/Data_images/Test_data.csv'
test_df=pd.read_csv(test_path)
test_df.columns = ['labels', 'filepaths']
# make filepaths column entries full path to the image
test_df['filepaths']=test_df['filepaths'].apply(lambda x: os.path.join(test_dir,x))
# make the entries in the labels column strings
...
print('class ', max_class, ' has the most images= ',max_value, '  class ', min_class, ' has the least images= ', min_value)
...
# <a id="trim"></a>
# # <center>Trim the the datasets</center>
# ### The train dataset is not balanced class 160 has only 156 images whereas class 2 has 2018 images
# ### To start to balance the dataset use the trim function to limit the maximum number of samples
# ### in a class to max_samples= 150 samples
# ### valid_df has 9743 images which is more than we need to use the trim function to limit the maximum
# ### samples in any class to 10
# ### the test_df has 53454 images far  more than needed so similarly limit it to have no more than
# ### 10 images per class.
# 
...
max_samples=10 # for test_df and valid_df
...
# ###  Given the small number (one class as low as 8 samples) of samples the results are very good
