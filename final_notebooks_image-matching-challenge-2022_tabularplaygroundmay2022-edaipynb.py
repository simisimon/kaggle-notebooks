...
# lets check out all letters and try to find unique letters as column name
...
for i in items:
    encoding = []
    for y in i:
        encoding.append(columns_name.index(y))
          
    f_27_pos.append(np.array(encoding))
    


train['f_27_pos'] = pd.Series(f_27_pos)
...
for i in items:
    ...
    f_27_pos.append(encoding)
    
f_27_series = pd.Series(f_27_pos)
train['f_27_pos'] = pd.Series(f_27_pos)
...
col_1 = pd.Series(col_1)
col_2 = pd.Series(col_2)
col_3 = pd.Series(col_3)
col_4 = pd.Series(col_4)
col_5 = pd.Series(col_5)
col_6 = pd.Series(col_6)
col_7 = pd.Series(col_7)
col_8 = pd.Series(col_8)
col_9 = pd.Series(col_9)

f_27_df = pd.concat([col_0, col_1, col_2,
                       col_3, col_4, col_5,
                       col_6, col_7, col_8,
                       col_9], axis=1)
...
cat_df = pd.DataFrame()
cat_df = cat_df.sparse.from_spmatrix(f_29_ohe, index=None, columns=None).astype(int)
cat_df = cat_df.sparse.from_spmatrix(f_30_ohe, index=None, columns=None).astype(int)
cat_df = cat_df.sparse.from_spmatrix(f_27_ohe, index=None, columns=None).astype(int)
cat_df = cat_df.sparse.from_spmatrix(f_27_l_ohe, index=None, columns=None).astype(int)
cat_df = cat_df.sparse.from_spmatrix(f_27_h_ohe, index=None, columns=None).astype(int)
cat_df.head(10)
...
train_pandas = pd.concat([int_pandas, float_pandas, cat_df, f_27_df], axis=1)
#train_pandas['f_27_pos'] = train['f_27_pos']
train_pandas.head()
...
'''
0.649
{'activation': 'identity', 'alpha': 0.1, 'hidden_layer_sizes': (71, 142), 'learning_rate': 'constant', 'solver': 'lbfgs'}
...
CLF = MLPClassifier(hidden_layer_sizes=(71, ),
                    max_iter=1500)
CLF.fit(X_train, y_train)
print(CLF.score(X_train, y_train))
print(CLF.score(X_test, y_test))
...
plt.title(f"{X_train.shape[0]}", fontsize=20)
...
plt.plot([0, 3], [0, 3], "darkorange", lw=2)
plt.xlim(0, 3)
plt.ylim(0, 3)
plt.xlabel("Y test", fontsize=16)
plt.ylabel("Predicted Y", fontsize=16)
...
cnf_matrix = confusion_matrix(y_test, CLF.predict(X_test), labels=[0, 1])
#cnf_matrix = confusion_matrix(y_test, y_pred, labels=[0, 1])
np.set_printoptions(precision=2)
plt.figure(figsize=(12, 5))
plot_confusion_matrix(cnf_matrix, classes=[0, 1])
...
'''parameter_space = {
    'hidden_layer_sizes': [(71,), (71,142), (71,32)],
...
GRD = GridSearchCV(CLF, parameter_space, n_jobs=-1, cv=2)
GRD.fit(X_train, y_train)
print(CLF.score(X_train, y_train))
print(CLF.score(X_test, y_test))


train_score = f'NN -- train -- {GRD.score(X_train, y_train)}'
test_score = f'NN -- test -- {GRD.score(X_test, y_test)}'
